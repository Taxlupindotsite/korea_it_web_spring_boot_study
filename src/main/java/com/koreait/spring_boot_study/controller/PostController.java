...
/* controller는 두가지 방식이 존재함
    1.  controller
html(웹페이지)를 반환하는 서버 사이드 렌더링(SSR)

    2. RestController
JSON, 문자열 등 다양한 데이터를 반환 => Rest Api 서버(웹서버)

//  서버 사이드 렌더링(SSR)이 뭐냐?
서버측에서 프론트의 웹페이지까지 통째로 반환, 그걸 브라우저에서 띄움
...
이러한 조합 => Single Page Application(SPA)방식 이라고 함
...
/*
    ...
    필요한 곳이 있으면 꺼내서 넣어줌
    ioc컨테이너에서 해당 객체를 찾아서 자동으로 넣어주니깐 우리가 new할 필요가 없음
    이미 실행될 때 ioc컨테이너에 객체들이 생성되서 보관되어 있음
